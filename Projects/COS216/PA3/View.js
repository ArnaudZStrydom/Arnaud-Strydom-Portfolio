...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function fetchListingImages(listingId,listing) {
            ...
            if (xmlReq.readyState === XMLHttpRequest.DONE) {
                if (xmlReq.status === 200) {
                    ...
                    if (response.status === "success") {
                        ...
                        populateImageCarousel(images,listing);
                    } else {
                        console.error("Error fetching listing images:", response.message);
                    }
                } else {
                    console.error("Error fetching listing images. HTTP status:", xmlReq.status);
                }
            }
        };
        ...
        xmlReq.open("GET", `https://wheatley.cs.up.ac.za/api/getimage?listing_id=${listingId}`, true);
        xmlReq.send();
    }
