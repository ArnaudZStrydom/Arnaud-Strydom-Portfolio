...
void printMatrix(float **matrix, int rows, int cols) {
    ...
    for (int i = 0; i < rows; i++) {
        if (matrix[i] == NULL) {
            printf("Error: Matrix row pointer is NULL\n");
            return;
        }
...
printf("I started scalar\n");
   
    multiplyScalarToMatrix(matrix1, scalar, rows, cols);
    printf("I got to the end of scalar\n");
...
printf("Just finished scalar\n");
    float **resultMatrix = addMatrices(matrix1, matrix2, rows, cols);
printf("done with scalar,now multiplying\n");
